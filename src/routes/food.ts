...
foodRoutes.post("/", async (req: Request, res: Response, next) => {
    try {
        const foodExists = await FoodModel.exists({food_name: req.body.food_name});
...
foodRoutes.put("/", async (req: Request, res: Response) => {
    try {
        ...
        if (op.acknowledged && op.modifiedCount == 1) {
            res.status(200)
                .send(`${op.modifiedCount} Resource food with name ${req.body.name} updated`)
                .end();
        } else {
            res.status(404)
...
foodRoutes.get("/search", async (req: Request, res: Response) => {
    try {
        ...
        const search = req.query.name ? new RegExp(`${req.query.name}`) : new RegExp('[a-zA-Z]');
